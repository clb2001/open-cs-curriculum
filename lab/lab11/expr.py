...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        "*** YOUR CODE HERE ***"
        # all are right
        # return self.operator.eval(global_env).apply([i.eval(env) for i in self.operands if i.eval(env) is not None])
        return self.operator.eval(global_env).apply([i.eval(env) for i in self.operands])
