...
def scaled_dot_product_two_loop_single(
    ...
    # 完全就是按照《模式识别与机器学习》这本书上来的，不知道下面这个实现怎么错了
    # for i in range(K):
    #     temp = torch.zeros((M, M), dtype=key.dtype, device=key.device)
    #     for j in range(M):
    #         temp[j] = torch.matmul(key[:, j], query) / (M ** 0.5)
    #     out[i] = torch.matmul(value[i], F.softmax(temp, dim=1))
    
    for i in range(K):
        temp = torch.zeros((K, K), dtype=key.dtype, device=key.device)
        for j in range(K):
            temp[j] = torch.matmul(key, query[j]) / (M ** 0.5)
        attention_weights = F.softmax(temp, dim=1)
        out[i] = torch.matmul(attention_weights[i], value)

    # # 下面是不用循环的做法
    # scaled_dot_product = torch.mm(query, key.t()) / (M ** 0.5)
    # attention_weights = F.softmax(scaled_dot_product, dim=1)
    # out = torch.mm(attention_weights, value)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_two_loop_batch(
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    temp = torch.zeros((N, K, K), dtype=key.dtype, device=key.device)
    for n in range(N):
        for i in range(K):
            temp[n, i] = torch.matmul(key[n], query[n, i]) / (M ** 0.5)
    attention_weights = F.softmax(temp, dim=2)
...
def scaled_dot_product_no_loop_batch(
    ...
    # the weights that dont need to be masked at that position. These masked-scaled#
...
class SelfAttention(nn.Module):
    ...
    def forward(
        ...
        #  three functions initialized above. Then, pass these three transformed #
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        # 一种更简便的写法
        # torch.nn.init.xavier_uniform_(self.linear_attn.weight.data)             
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class EncoderBlock(nn.Module):
    def __init__(
       ...
       inp - multi_head_attention - out1 - layer_norm(out1 + inp) - dropout - out2 \ 
        ...
        self.feedforard_block = FeedForwardBlock(emb_dim, feedforward_dim)
    ...
    def forward(self, x):
        ...
        out3 = self.feedforard_block(out2)
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    y = torch.linspace(0, 1 - 1 / K, steps=K).view(-1, 1).expand(1, K, M)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return y
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    #     for m in range(M):
    #         if m % 2 == 0:
    #             y[0, k, m] = torch.sin(torch.tensor(k/10000**(m//M)))
    #         else:
    #             y[0, k, m] = torch.cos(torch.tensor(k/10000**((m-1)//M)))
    # 改进后的算法：
    k = torch.arange(K).view(-1, 1)
    m = torch.arange(M).view(1, -1)
    # 有个问题，m // M始终是0，这样算m // M有什么意义？
    angles = k / 10000 ** (m // M)
    y = torch.zeros((1, K, M))
    y[0, :, 0::2] = torch.sin(angles[:, 0::2])
    y[0, :, 1::2] = torch.cos(angles[:, 1::2])
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return y
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        args:
            ...
            M is the input embedding dimension.
        returns:
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        enp_out = self.encoder(q_emb_inp)       
        mask = get_subsequent_mask(ans_b[:, :-1])
        N, O, _ = mask.shape
        dec_out = self.decoder(a_emb_inp, enp_out, mask).view(N * O, -1)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class AddSubDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        """
        The core function to get element with index idx in the data.
        args:
