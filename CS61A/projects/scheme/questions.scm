...
(define (enumerate s)
  ...
  (cond ((null? remainder-s) remainder-s)
    (else (cons (cons n (cons (car remainder-s) nil)) (enumerate-from (+ 1 n) (cdr remainder-s))))))
  (enumerate-from 0 s)
  ; END PROBLEM 15
)
...
(define (nondecreaselist s)
  ; BEGIN PROBLEM 17
  (cond ((null? s) s)
        ((null? (cdr s)) (cons s nil)) ; not ((null? (cdr s)) s)
        ((<= (car s) (car (cdr s))) (cons (cons (car s) (car (nondecreaselist (cdr s)))) (cdr (nondecreaselist (cdr s)))))
        (else (cons (cons (car s) nil) (nondecreaselist (cdr s)))) ; (car s) is a number, should build it as a pair
  )
)
  ; END PROBLEM 17
